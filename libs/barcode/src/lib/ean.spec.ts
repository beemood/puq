import { ean, validateEAN13 } from './ean.js';
...
describe('ean() EAN-13 Generation', () => {
  ...
  test.each([
    ...
    expect(() => ean(invalidPrefix as any)).toThrow();
  });
});
...
describe('validateEAN13() EAN-13 Validation', () => {
  test.each([
    ...
    expect(validateEAN13(eanCode as string)).toBe(expected);
  });
});
